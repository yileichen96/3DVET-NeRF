...
def config_parser():
    ...
    parser.add_argument("--dataset_dir", type=str, default="/data/yilechen/Dataset/IBRNet_Data/",
    ...
    ## model options
    parser.add_argument(
        "--conv_feature_dim", type=int, default=32, help="2D feature dimension for easy conv"
    )
    parser.add_argument(
        "--head", type=int, default=4, help="number of transformer heads"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--cross_depth", type=int, default=1, help="network intermediate dimension"
    )
    parser.add_argument(
        "--nerf_width", type=int, default=128, help="MLP width for condition nerf"
    )
    parser.add_argument(
        "--nerf_depth", type=int, default=6, help="MLP depth for condition nerf"
    )
    parser.add_argument(
        "--nerf_skip", type=list, default=[4], help="skip connections"
    )
    parser.add_argument(
        "--num_source_views",
        type=int,
    ...
    parser.add_argument(
        "--vis_depth",
        action="store_true",
        help="will return density to infer depth",
    )
    ...
    parser.add_argument("--lrate_eve", type=float, default=5e-4, help="learning rate for evenerf")
    parser.add_argument(
        "--lrate_decay_factor",
        type=float,
        default=0.5,
    ...
    ########## rendering options ##########
    parser.add_argument(
        "--N_samples", type=int, default=64, help="number of nerf samples per ray"
    )
    parser.add_argument(
        "--inv_uniform", action="store_true", help="if True, will uniformly sample inverse depths"
    )
    parser.add_argument(
        "--det", action="store_true", help="deterministic sampling for nerf samples"
    )
    parser.add_argument(
        "--white_bkgd",
        action="store_true",
