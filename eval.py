...
def eval(args):
    ...
    for k, data in enumerate(tqdm.tqdm(loader, leave=True)):
        if args.local_rank == 0:
            ...
            idx += 1
            if idx >= scene_image_count[current_scene_id.item()]:
                idx = 0
                print("(Scene {}) psnr: {:.4f}, ssim: {:.4f}, lpips: {:.4f}".format(current_scene_id.item(),
                                                                                    np.mean(image_psnr_list),
                                                                                    np.mean(image_ssim_list),
                                                                                    np.mean(image_lpips_list)))
                dataframe_lfi = pd.DataFrame({'Scene {}'.format(current_scene_id.item()): image_list,
                ...
                scene_list.append(current_scene_id.item())
...
def log_view(
    ...
    filename = os.path.join(out_folder, prefix[:-1] + "Scene{:d}_{:03d}.png".format(scene_id.item(), global_step))
    ...
    if vis_depth:
        ...
        depth_show = img_HWC2CHW(
            colorize(depth_pred, range=(ray_sampler.depth_vis_range[0].item(), ray_sampler.depth_vis_range[1].item()),
                     cmap_name="jet"))
        depth_show = depth_show.permute(1, 2, 0).detach().cpu().numpy()
        filename = os.path.join(
            out_folder, prefix[:-1] + "Scene{:d}_{:03d}_depth.png".format(scene_id.item(), global_step)
        )
