...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None, pose=None, feats_3d=None):
        if pose is not None:
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_head, d_model, d_k, d_v, dropout=0.1, d_pose=None, d_3d=None):
        super().__init__()
        ...
        if d_3d is not None:
            self.attn_norm_3d = nn.LayerNorm(d_3d, eps=1e-6)
            self.w_qs = nn.Linear(d_model + d_3d, n_head * d_k, bias=False)
        else:
    ...
    def forward(self, q, k, v, mask=None, pose=None, feats_3d=None):
        ...
        if feats_3d is not None:
            feats_3d = self.attn_norm_3d(feats_3d)
            q = self.w_qs(torch.concat([q, feats_3d], dim=2)).view(sz_b, len_q, n_head, d_k)
        else:
        ...
        q, attn = self.attention(q, k, v, mask=mask, pose=pose, feats_3d=feats_3d)
...
class ScaledDotProductAttention_v1(nn.Module):
    ...
    def forward(self, q, k, v, q_3d, k_3d, weight_3d, mask=None, pose=None, feats_3d=None):
        if pose is not None:
            attn = torch.matmul((q + pose) / self.temperature, k.transpose(2, 3))
        else:
            attn = torch.matmul(q / self.temperature, k.transpose(2, 3))

        attn_3d = torch.matmul(q_3d / self.temperature, k_3d.transpose(2, 3))

        attn = attn + weight_3d * attn_3d
...
class MultiHeadAttention_v1(nn.Module):
    ...
    def __init__(self, n_head, d_model, d_k, d_v, dropout=0.1, d_pose=None, d_3d=None):
        ...
        self.w_qs_3d = nn.Linear(d_model, n_head * d_k, bias=False)
        self.w_ks_3d = nn.Linear(d_model, n_head * d_k, bias=False)
        ...
        self.attention = ScaledDotProductAttention_v1(temperature=d_k ** 0.5)
        ...
        self.weight = nn.Sequential(nn.Linear(d_model * 2, n_head * 1),nn.ReLU(inplace=True))
    ...
    def forward(self, q, k, v, mask=None, pose=None, feats_3d=None):
        ...
        weight_3d = F.sigmoid(self.weight(torch.cat([v, feats_3d], dim=-1))).view(sz_b, len_q, n_head, 1)
        weight_3d = weight_3d.transpose(1, 2)

        residual = q
        q = self.attn_norm(q)
        q_3d = self.attn_norm_3d(feats_3d)
        k_3d = feats_3d
        ...
        q_3d = self.w_qs_3d(q_3d).view(sz_b, len_q, n_head, d_k)
        k_3d = self.w_ks_3d(k_3d).view(sz_b, len_k, n_head, d_k)
        ...
        # Transpose for attention dot product: b x n x lq x dv
        q, k, v = q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2)
        q_3d, k_3d = q_3d.transpose(1, 2), k_3d.transpose(1, 2)

        if mask is not None:
            mask = mask.unsqueeze(1)   # For head axis broadcasting.

        q, attn = self.attention(q, k, v, q_3d, k_3d, weight_3d, mask=mask, pose=pose)
