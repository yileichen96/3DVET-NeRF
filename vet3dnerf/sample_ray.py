...
class RaySamplerSingleImage(object):
    def __init__(self, data, device, resize_factor=1, render_stride=1):
        ...
        # )
        self.rays_o, self.rays_d, self.pixel_coords = self.get_rays_single_image_and_coord(
            self.H, self.W, self.intrinsics, self.c2w_mat
        )
        if self.rgb is not None:
        ...
        if "proj_mats" in data.keys():
            self.proj_mats = data["proj_mats"]
        else:
            self.proj_mats = None
        if "depth_vis_range" in data.keys():
            self.depth_vis_range = data["depth_vis_range"]
        else:
            self.depth_vis_range = None
    ...
    def get_rays_single_image_and_coord(self, H, W, intrinsics, c2w):
        """
        :param H: image height
        :param W: image width
        :param intrinsics: 4 by 4 intrinsic matrix
        ...
        return rays_o, rays_d, batched_pixels.transpose(1, 2).reshape(-1, 3)
    ...
    def get_all(self):
        ret = {
            ...
            "proj_mats": self.proj_mats.cuda() if self.proj_mats is not None else None,
            "pixel_coords": self.pixel_coords.cuda() if self.pixel_coords is not None else None,
            "depth_vis_range": self.depth_vis_range if self.depth_vis_range is not None else None,
        }
        return ret
    ...
    def random_sample(self, N_rand, sample_mode, center_ratio=0.8):
        ...
        if self.pixel_coords is not None:
            pixel_coords = self.pixel_coords[select_inds]
        else:
            pixel_coords = None
        ...
        ret = {
            ...
            "proj_mats": self.proj_mats.cuda() if self.proj_mats is not None else None,
            "pixel_coords": pixel_coords.cuda() if pixel_coords is not None else None,
        }
        return ret
