...
from .data_utils import quaternion_about_axis, quaternion_matrix, random_crop, random_flip, parse_camera_pre
import json
import math
...
class SpacesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if np.random.choice([0, 1], p=[0.5, 0.5]) and self.mode == "train":
            target_rgb, target_camera, ref_rgbs_np, ref_cameras = random_flip(
                target_rgb, target_camera, ref_rgbs_np, ref_cameras
            )
        ...
        return {
            "rgb": torch.from_numpy(target_rgb).float(),
...
class SpacesFreeDataset(Dataset):
    def __init__(self, args, mode, **kwargs):
        self.folder_path = os.path.join(args.dataset_dir, "TrainingSet/spaces_dataset/data/800/")
        self.mode = mode
        self.num_source_views = args.num_source_views
    ...
    def __getitem__(self, idx):
        ...
        # num_select = 20
        # nearby_view_ids_selected = np.random.choice(
        #     sorted_ids[1:], min(num_select,len(sorted_ids)), replace=False
        # )
        ...
        # computing proj_mats
        W, H, intrinsics, c2w_target = parse_camera_pre(render_camera)
        ...
        proj_mats = []
        for id in range(len(nearby_view_ids_selected)):
            # build proj mat from source views to target view
            W, H, intrinsics, c2w = parse_camera_pre(ref_cameras[id])
