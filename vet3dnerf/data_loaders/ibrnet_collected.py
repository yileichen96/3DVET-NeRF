...
sys.path.append("../")
from .data_utils import rectify_inplane_rotation, random_crop, random_flip, get_nearest_pose_ids, parse_camera_pre
from .llff_data_utils import load_llff_data, batch_parse_llff_poses


class IBRNetCollectedDataset(Dataset):
    def __init__(self, args, mode, random_crop=True, **kwargs):
        self.folder_path1 = os.path.join(args.dataset_dir, "TrainingSet/ibrnet_collected_1/")
        self.folder_path2 = os.path.join(args.dataset_dir, "TrainingSet/ibrnet_collected_2/")
        ...
        with open(os.path.join(self.folder_path1, 'scene_list.txt'), 'r') as file:
            scene_names1 = file.read().splitlines()
        scenes1 = [os.path.join(self.folder_path1, scene_name) for scene_name in scene_names1]
        with open(os.path.join(self.folder_path2, 'scene_list.txt'), 'r') as file:
            scene_names2 = file.read().splitlines()
        scenes2 = [os.path.join(self.folder_path2, scene_name) for scene_name in scene_names2]
